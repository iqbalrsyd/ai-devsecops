...
# This is the REAL test - if this works, webapp should work too
...
# Config
GITHUB_TOKEN="${GITHUB_TOKEN:-}"  # Get from .env if set
...
if [ -z "$GITHUB_TOKEN" ]; then
    echo "WARNING: No GITHUB_TOKEN in .env. Using empty string (will fail for private repos)."
fi
...
UPDATE users SET password_hash = crypt('TestPass123!', gen_salt('bf')) WHERE email = '$USER_EMAIL';" 2>&1 | tail -1
...
try:
    ...
    if 'pipeline' in data:
        ...
        yaml = p.get('generated_yaml', '')
        print(f'  YAML: {len(yaml)} chars')
        if yaml:
            print('  First 200 chars:', repr(yaml[:200]))
...
if [ $NEW_AI -gt 0 ] && [ $NEW_AI -lt 100 ]; then
    docker logs ai-devsecops-ai-service-1 --tail=$NEW_AI 2>&1 | head -50
fi
